...
{
    [Range(100,300)]
    public float GameSpeed=100; // задаем ограничени€ скорости
    ...
    private void Start()
	{
       ...
       if(SceneManager.GetActiveScene().buildIndex==1&& Time_Now!=null) // если присутствует скайбокс и мы в сцене игры
                                                                        // - тогда прнидаем скорость, разворачиваемс€ и мен€ем фон на заданный
    ...
    public void InputName()
    {
		...
		else
		{
            ...
            PlayerPrefs.SetString("Name", Input_Field.text);// фиксируем в базе измененное им€
